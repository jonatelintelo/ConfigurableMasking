...
if __name__ == "__main__":
    ...
    def create_ablation_figure(data_subset, rows, cols, figsize, filename_suffix):
    ...
    create_ablation_figure(df_main, rows=1, cols=3, figsize=(7, 2.2), filename_suffix=F"{MODE}_ablation_main")

    print("\nGenerating ACM CCS Appendix Plot...")
    df_appendix = df[df["in_main_body"] == False]
    if not df_appendix.empty:
        create_ablation_figure(df_appendix, rows=1, cols=2, figsize=(7, 2.2), filename_suffix=F"{MODE}_ablation_appendix_ccs")
    else:
