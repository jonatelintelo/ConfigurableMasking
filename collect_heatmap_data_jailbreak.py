...
if __name__ == "__main__":
    ...
    def get_topk_hook(layer_idx):
        def hook(module, input, output):
            topk = output[0] if isinstance(output, (tuple, list)) else torch.topk(output, k=model_config.top_k, dim=-1, sorted=False).indices.cpu()
            current_batch_topk[layer_idx] = topk.detach().cpu()
    ...
    for layer_name, module in model.named_modules():
        if layer_name.lower().endswith(model_config.gate_name.lower()):
            ...
            get_topk_hooks.append(module.register_forward_hook(get_topk_hook(layer_idx)))
            layer_idx += 1
    ...
    for b_idx, batch_prompts in enumerate(
        ...
        for p_idx in range(b_size):
            ...
            prompt_topk = [current_batch_topk[l_idx][p_idx, start : end + 1, :] for l_idx in range(layer_idx)]
            stacked_topk = torch.stack(prompt_topk, dim=1)
            all_topks.append(stacked_topk.cpu())

        flush()

    for h in collection_handles + get_topk_hooks:
        h.remove()

    os.makedirs(os.path.join(root_folder, "results", "topk", "jailbreak"), exist_ok=True)
    baseline_path = os.path.join(root_folder, "results", "topk", "jailbreak", f"{model_config.model_name}_baseline_topk.pt")
    steered_path = os.path.join(root_folder, "results", "topk", "jailbreak", f"{model_config.model_name}_steered_topk.pt")

    torch.save(all_topks, baseline_path)
    ...
    for config in configs_to_test:
        ...
        for steering_alpha in config["alphas"]:
            ...
            for b_idx, batch_prompts in enumerate(
                tqdm(data_utils.batchify(prompts, BATCH_SIZE), total=(len(prompts) + BATCH_SIZE - 1) // BATCH_SIZE, desc="Steered TopK Collection")
            ):
                current_batch_topk.clear()
                inputs = tokenizer(batch_prompts, return_tensors="pt", padding=True, truncation=True, return_offsets_mapping=True)
                offset_mappings = inputs.pop("offset_mapping").cpu().numpy()
                ...
                for p_idx in range(b_size):
                    ...
                    prompt_topk = [current_batch_topk[l_idx][p_idx, start : end + 1, :] for l_idx in range(layer_idx)]
                    stacked_topk = torch.stack(prompt_topk, dim=1)
                    all_steered_topks.append(stacked_topk.cpu())

                flush()

            for hook in active_steering_hooks + get_topk_hooks:
                hook.remove()

            torch.save(all_steered_topks, steered_path)

    baseline = torch.load(baseline_path)
    steered = torch.load(steered_path)

    # Compare first prompt, first token, all layers
    b_experts = set(baseline[0][0, 0, :].tolist())
    s_experts = set(steered[0][0, 0, :].tolist())

    intersection = b_experts.intersection(s_experts)
    print(f"Expert overlap for token 0: {len(intersection)} / {len(b_experts)}")
